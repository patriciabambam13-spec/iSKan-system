...
export default function RegisterYouth() {
  ...
  const initialForm = {
        ...
        zip_code:"1111", // Added zip code field with permanent value
        residency_status:"",
        years_residency:"",
        contact:"",
        email:"",
  ...
  async function sendEmailWithQR(youthID, youthName, youthEmail) {
    try {
      // Get the QR code as base64 image
  ...
  async function handleSubmit(e){
    ...
    try {
      ...
      // Step 1: Upload photo first (if exists)
      let photoUrl = null;
      if (photo) {
        try {
...
3. You have proper RLS policies set up`);
      ...
      // Step 2: Insert youth data into database
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          zip_code: formData.zip_code, // Added zip_code field
      ...
      // Step 5: Set QR code and show modal FIRST
      setQrCode(youthID);
      setShowQR(true);
      
      // Step 6: Wait for QR canvas to render, then send email with QR image
      setTimeout(async () => {
    ...
    <> 
      ...
      <form onSubmit={handleSubmit} className="register-form">
        ...
        <div className="section-card">
          ...
          {/* Grid layout for Years of Residency and ZIP Code */}
